...
def rank_tweets_by_sentiment(tweets):
    ...
    for tweet in tweets:
        ...
        positive_score = sentiment_score[-1]  # BERT sentiment model's positive class is the last one
        scores.append(positive_score)
    
    ranked_tweets = sorted(zip(tweets, scores), key=lambda x: x[1], reverse=True)
    return ranked_tweets

# Function to rank tweets based on relevance to the prompt using Sentence-BERT
